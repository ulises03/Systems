...
public class SPerfilesAccesosJpaController implements Serializable {
    ...
    public List<SPerfilesAccesos> findSperfilesAccesoEntitiesByPerfil(int id ){
        EntityManager em = getEntityManager();
        Query query = em.createNamedQuery("SPerfilesAccesos.findByIdPerfil");
        query.setParameter("idPerfil", id);
        List<SPerfilesAccesos> list = query.getResultList( );
        em.close();
        return list;
    }
